...
from mrcnn.model import log, load_image_gt
import cell
from trainingThread import CustomDataset
...
def find_image_index(image_infos, image_path):
    for i, image_info in enumerate(image_infos):
        if os.path.normpath(image_info['path']) == os.path.normpath(image_path):
            return i
...
# 定義工作函數
@ray.remote
def process_image(DEVICE, MODEL_DIR, weights_path, config, ROI_PATH, DETECT_PATH, image_path, j, conf_rate, epoches, step, dataset):
    ...
    # get ground truth bounding boxes and their corresponding class IDs from your dataset
    image_index = find_image_index(dataset.image_info, image_path)
    _, _, gt_class_id, gt_bbox, gt_mask = load_image_gt(dataset, config, image_index) # you need to implement this function
    
    
    # get predicted bounding boxes, class IDs, and scores
    r = results[0]
    rois = r['rois']
    ...
    # compute AP
    ap = mrcnn.utils.compute_ap(gt_bbox, gt_class_id, gt_mask, rois, class_ids, scores, masks)
    
    # you can return the AP value along with any other information you need
    return ap
...
class detectingThread(QtCore.QThread):
    ...
    def run(self):
        class CustomConfig(Config):
            ...
            DETECTION_MIN_CONFIDENCE = float(self.confidence)
        ...
        for j, filename in enumerate(filenames):
            ...
            futures.append(process_image.remote(
                DEVICE, MODEL_DIR, weights_path, config, 
             self.ROI_PATH, self.DETECT_PATH,
                filename, j, self.conf_rate, 
                self.epoches, self.step,dataset_val))
            
        for j in tqdm(to_iterator(futures), total=len(futures)):
            self.progressBar.emit(j)
        mAPs = ray.get(futures)
        # 其他程式碼...
        print("The accuracy is ", np.mean(mAPs))
        # 關閉 Ray
        ray.shutdown()
