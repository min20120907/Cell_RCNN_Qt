...
class DataGenerator(KU.Sequence):
    ...
    def __getitem__old(self, idx):
        ...
        # Submit load_image_gt jobs to ray cluster
        futures = [load_image_gt_remote.remote(
            self.dataset, self.config, image_id, self.augmentation) for image_id in image_ids]

        # Retrieve results from ray cluster
        ...
        for i, result in enumerate(results):
            ...
            batch_rpn_match.append(rpn_match[:, np.newaxis])
            batch_rpn_bbox.append(rpn_bbox)
            batch_images.append(mold_image(image.astype(np.float32), self.config))
            ...
            if self.random_rois:
                ...
                if self.detection_targets:
                    ...
                    batch_rpn_rois.append(rpn_rois)

        # Convert batch arrays to numpy arrays
        batch_image_meta = np.stack(batch_image_meta)
        batch_rpn_match = np.stack(batch_rpn_match)
        batch_rpn_bbox = np.stack(batch_rpn_bbox)
        batch_images = np.stack(batch_images)
        batch_gt_class_ids = np.stack(batch_gt_class_ids)
        batch_gt_boxes = np.stack(batch_gt_boxes)
        batch_gt_masks = np.stack(batch_gt_masks)

        if self.random_rois:
            batch_mrcnn_bbox = np.stack(batch_mrcnn_bbox)
            batch_mrcnn_mask = np.stack(batch_mrcnn_mask)
            batch_rois = np.stack(batch_rois)
            batch_rpn_rois = np.stack(batch_rpn_rois)
        inputs = [batch_images, batch_image_meta, batch_rpn_match, batch_rpn_bbox,
                  batch_gt_class_ids, batch_gt_boxes, batch_gt_masks]
        outputs = []
...
class MaskRCNN(object):
    ...
    def set_trainable(self, layer_regex, keras_model=None, indent=0, verbose=1):
        ...
        # Build list of trainable layers
        trainable_layers = [layer for layer in layers if bool(re.fullmatch(layer_regex, layer.name))]

        # Set trainable attribute for all matching layers
        ...
        if verbose > 0:
            trainable_layer_names = [layer.name for layer in trainable_layers]
